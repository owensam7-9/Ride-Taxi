 ...
 // eslint-disable-next-line
  ...
  useEffect(() => {
    ...
    const calculateRoute = async () => {
      if (!isPickupDisable || !isDestinationDisable || !origin || !end) {
        return;
      }
    ...
    if (origin && end && isPickupDisable && isDestinationDisable) {
      calculateRoute();
    }
  ...
  return (
    <div className={`bg-gray-300 h-[45vh] sm:h-[94vh] w-full`}>
      {isLoadingDirections && (
        <div className="absolute top-0 left-0 right-0 z-50 bg-blue-100 py-2 text-center text-blue-600">
          Calculating route...
        </div>
      )}
      <GoogleMap
